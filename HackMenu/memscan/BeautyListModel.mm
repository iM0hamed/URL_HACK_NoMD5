                   ...
                   dispatch_async(queue, ^{
                       JRMemoryEngine engine = JRMemoryEngine(mach_task_self());
                        ...
                        if(jingquesousuodata!=nil){
                           if ([jingquegeshidata isEqual:@"Double"]) {
                           double search5 = [jingquesousuodata doubleValue];
                           engine.JRScanMemory(range, &search5, JR_Search_Type_Double);
                       }
                           if ([jingquegeshidata isEqual:@"Float"]) {
                           float search5 = [jingquesousuodata floatValue];
                           engine.JRScanMemory(range, &search5, JR_Search_Type_Float);
                       }
                           if ([jingquegeshidata isEqual:@"I8"]) {
                           SInt8 search5 = [jingquesousuodata intValue];
                           engine.JRScanMemory(range, &search5, JR_Search_Type_SByte);
                       }
                           if ([jingquegeshidata isEqual:@"I16"]) {
                           SInt16 search5 = [jingquesousuodata intValue];
                           engine.JRScanMemory(range, &search5, JR_Search_Type_SShort);
                       }
                           if ([jingquegeshidata isEqual:@"I32"]) {
                           SInt32 search5 = [jingquesousuodata intValue];
                           engine.JRScanMemory(range, &search5, JR_Search_Type_SInt);
                       }
                           if ([jingquegeshidata isEqual:@"I64"]) {
                           SInt64 search5 = [jingquesousuodata longLongValue];
                           engine.JRScanMemory(range, &search5, JR_Search_Type_SLong);
                       }
//                           if ([jingquegeshidata isEqual:@"U8"]) {
//                           uint8_t search5 = [jingquesousuodata unsignedIntValue];
//                           engine.JRScanMemory(range, &search5, JR_Search_Type_UByte);
//                       }
//                           if ([jingquegeshidata isEqual:@"U16"]) {
//                           uint16_t search5 = [jingquesousuodata unsignedIntValue];
//                           engine.JRScanMemory(range, &search5, JR_Search_Type_UShort);
//                       }
//                           if ([jingquegeshidata isEqual:@"U32"]) {
//                           uint32_t search5 = [jingquesousuodata unsignedIntValue];
//                           engine.JRScanMemory(range, &search5, JR_Search_Type_UInt);
//                       }
//                           if ([jingquegeshidata isEqual:@"U64"]) {
//                           uint64_t search5 = [jingquesousuodata unsignedLongLongValue];
